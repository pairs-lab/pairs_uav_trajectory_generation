...
# ---------------- window: random_flier ----------------
read W_random_flier P <<< "$(tmux new-window -t "$SESSION_NAME" -n "random_flier" -P -F '#{window_id} #{pane_id}')"
tmux send-keys -t "$P" "$SETUP; "'waitForHw; roslaunch pairs_uav_trajectory_generation path_random_flier.launch custom_config:=./config/path_random_flier.yaml' Enter
tmux select-layout -t "$W_random_flier" tiled
...
# ---------------- window: rqt_reconfigure ----------------
read W_rqt_reconfigure P <<< "$(tmux new-window -t "$SESSION_NAME" -n "rqt_reconfigure" -P -F '#{window_id} #{pane_id}')"
tmux send-keys -t "$P" "$SETUP; "'waitForControl; rosrun rqt_reconfigure rqt_reconfigure' Enter
tmux select-layout -t "$W_rqt_reconfigure" tiled
