...
private:
  ...
  bool   setPathSrv(const pairs_msgs::Path path_in);
  ...
  std::optional<pairs_msgs::TrackerCommand> transformTrackerCmd(const pairs_msgs::TrackerCommand& tracker_cmd, const std::string& target_frame);
...
void PathRandomFlier::onInit(void) {
  ...
  pairs_lib::ParamLoader param_loader(nh_, "PathRandomFlier");
  ...
  sh_tracker_cmd_          = pairs_lib::SubscribeHandler<pairs_msgs::TrackerCommand>(shopts, "tracker_command_in");
  ...
  service_client_path_            = nh_.serviceClient<pairs_msgs::PathSrv>("path_out");
  service_client_check_reference_ = nh_.serviceClient<pairs_msgs::ValidateReference>("check_reference_out");
  ...
  transformer_ = std::make_shared<pairs_lib::Transformer>(nh_, "PathRandomFlier");
...
void PathRandomFlier::timerMain([[maybe_unused]] const ros::TimerEvent& event) {
  ...
  if (!tracker_cmd_transformed) {
    ...
    ROS_ERROR_STREAM("[PairsTrajectoryGeneration]: " << ss.str());
    return;
  }
  ...
  if ((ros::Time::now() - last_successfull_command_).toSec() > 1.0 &&
    ...
    if (!next_wait_for_finish_) {
      ...
      pairs_msgs::ReferenceStamped new_point;
    ...
    for (int it = 0; it < n_points; it++) {
      ...
      pairs_msgs::Reference new_point;
...
bool PathRandomFlier::setPathSrv(const pairs_msgs::Path path_in) {
...
std::optional<pairs_msgs::TrackerCommand> PathRandomFlier::transformTrackerCmd(const pairs_msgs::TrackerCommand& tracker_cmd, const std::string& target_frame) {
  ...
  if (!tf) {
    ROS_ERROR("[PairsTrajectoryGeneration]: could not find transform from '%s' to '%s' in time %f", tracker_cmd.header.frame_id.c_str(), target_frame.c_str(),
              tracker_cmd.header.stamp.toSec());
    return {};
  }

  pairs_msgs::TrackerCommand cmd_out;
...

  {
    ...
    if (auto ret = transformer_->transform(pos, tf.value())) {
      ...
      try {
        ...
        ROS_ERROR("[PairsTrajectoryGeneration]: failed to transform heading in tracker_cmd");
        cmd_out.heading = 0;
      }
    } else {
      return {};
...
PLUGINLIB_EXPORT_CLASS(pairs_uav_trajectory_generation::PathRandomFlier, nodelet::Nodelet)
